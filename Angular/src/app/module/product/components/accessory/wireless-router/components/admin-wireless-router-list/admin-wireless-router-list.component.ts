...
@Component({
  selector: 'admin-wireless-router-list',
  templateUrl: './admin-wireless-router-list.component.html',
  styleUrls: ['./admin-wireless-router-list.component.css']
})
export class AdminWirelessRouterListComponent implements OnInit {
  ...
  constructor(private router: Router, private wirelessRouterService: WirelessRouterService,
   ...
   */
  countAll() {
    this.wirelessRouterService.countAll().subscribe(quantity => {
   ...
   */
  countByFilter() {
    this.wirelessRouterService.countByFilter(this.keySearch, this.brandName, this.priceFrom, this.priceTo).subscribe(quantity => {
      this.totalItems = quantity;
    }, error => {
      console.log(error);
    });
  ...
  getPerPage(page: number) {
    this.wirelessRouterService.getPerPage(page).subscribe(wirelessRouters => {
      this.pagedItems = wirelessRouters;
  ...
  getByFilter(page: number) {
    this.countByFilter();
    this.wirelessRouterService.getByFilter(this.keySearch, this.brandName, this.priceFrom, this.priceTo, page).subscribe(wirelessRouters => {
      this.pagedItems = wirelessRouters;
   ...
   */
  getBrandList() {
    this.brandService.getAllByProductType('Bộ định tuyến').subscribe(brands => {
      this.brands = brands;
  ...
  delete() {
    ...
    swal({
      type: 'question',
      title: 'Bạn có chắc muốn xoá bộ định tuyến có mã ' + idList + ' ?',
      ...
      if (result.value) {
        if (this.idList.length == 1) {   // Delete one product
          this.wirelessRouterService.delete(this.idList[0]).subscribe(data => {
            swal({
              type: 'success',
              title: 'Đã xoá bộ định tuyến có mã ' + idList + ' thành công!'
            });
            this.countAll();
          }, error => {
            swal({
          ...
          for (var i = 0; i < this.idList.length; i++) {
            this.wirelessRouterService.delete(this.idList[i]).subscribe(data => {
            }, error => {
              swal({
                type: 'error',
                title: 'Xoá thất bại!'
  ...
  seeDetail(name: string) {
    this.router.navigate(['/admin/accessories/wireless-routers/', name]);
  }
