...
@Component({
  selector: 'admin-rice-cooker-list',
  templateUrl: './admin-rice-cooker-list.component.html',
  styleUrls: ['./admin-rice-cooker-list.component.css']
})
export class AdminRiceCookerListComponent implements OnInit {
  ...
  constructor(private router: Router, private riceCookerService: RiceCookerService,
   ...
   */
  countAll() {
    this.riceCookerService.countAll().subscribe(quantity => {
   ...
   */
  countByFilter() {
    this.riceCookerService.countByFilter(this.keySearch, this.brandName, this.priceFrom, this.priceTo).subscribe(quantity => {
      this.totalItems = quantity;
    }, error => {
      console.log(error);
    });
  ...
  getPerPage(page: number) {
    this.riceCookerService.getPerPage(page).subscribe(riceCookers => {
      this.pagedItems = riceCookers;
  ...
  /**
   * Get list of rice cookers by filter
   * 
   * @since 17/11/2018
   * @param page
   */
  getByFilter(page: number) {
    this.countByFilter();
    this.riceCookerService.getByFilter(this.keySearch, this.brandName, this.priceFrom, this.priceTo, page).subscribe(riceCookers => {
      this.pagedItems = riceCookers;
   ...
   */
  getBrandList() {
    this.brandService.getAllByProductType('Nồi cơm điện').subscribe(brands => {
      this.brands = brands;
  ...
  delete() {
    ...
    swal({
      type: 'question',
      title: 'Bạn có chắc muốn xoá nồi cơm điện có mã ' + idList + ' ?',
      ...
      if (result.value) {
        if (this.idList.length == 1) {   // Delete one product
          this.riceCookerService.delete(this.idList[0]).subscribe(data => {
            swal({
              type: 'success',
              title: 'Đã xoá nồi cơm điện có mã ' + idList + ' thành công!'
            });
            this.countAll();
          }, error => {
            swal({
          ...
          for (var i = 0; i < this.idList.length; i++) {
            this.riceCookerService.delete(this.idList[i]).subscribe(data => {
            }, error => {
              swal({
                type: 'error',
                title: 'Xoá thất bại!'
  ...
  seeDetail(name: string) {
    this.router.navigate(['/admin/appliances/rice-cookers/', name]);
  }
