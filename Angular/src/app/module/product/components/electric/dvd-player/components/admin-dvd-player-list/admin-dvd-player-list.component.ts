...
@Component({
  selector: 'admin-dvd-player-list',
  templateUrl: './admin-dvd-player-list.component.html',
  styleUrls: ['./admin-dvd-player-list.component.css']
})
export class AdminDVDPlayerListComponent implements OnInit {
  ...
  constructor(private router: Router, private dvdPlayerService: DVDPlayerService,
  ...
  /**
   * Get list of DVD players by filter
   * 
   * @since 17/11/2018
   * @param page
   */
  ...
  delete() {
    ...
    swal({
      type: 'question',
      title: 'Bạn có chắc muốn xoá đầu đĩa DVD có mã ' + idList + ' ?',
      ...
      if (result.value) {
        if (this.idList.length == 1) {   // Delete one product
          this.dvdPlayerService.delete(this.idList[0]).subscribe(data => {
            swal({
              type: 'success',
              title: 'Đã xoá đầu đĩa DVD có mã ' + idList + ' thành công!'
            });
            this.countAll();
          }, error => {
            swal({
          ...
          for (var i = 0; i < this.idList.length; i++) {
            this.dvdPlayerService.delete(this.idList[i]).subscribe(data => {
            }, error => {
              swal({
                type: 'error',
                title: 'Xoá thất bại!'
  ...
  seeDetail(name: string) {
    this.router.navigate(['/admin/electrics/dvd-players/', name]);
  }
