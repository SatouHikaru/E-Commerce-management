...
export class AirConditionerDetailComponent implements OnInit {
  ...
  constructor(private activateRoute: ActivatedRoute, private router: Router,
      private airConditionerService: AirConditionerService, private customerService: CustomerService,
   ...
   */
  checkQuantity() {
    if (this.quantitySelected > this.airConditioner.quantity) {
      this.quantitySelected = this.airConditioner.quantity - 1;
    } else if (this.quantitySelected < 0) {
      if (this.airConditioner.quantity <= 0) {
    ...
    if (typeof this.quantitySelected == 'string') {
      if (this.airConditioner.quantity > 0) {
        if (this.airConditioner.quantity >= 1) {
          this.quantitySelected = 1;
        } else {
          this.quantitySelected = 0;
        }
   ...
   */
  add() {
    if (this.quantitySelected < this.airConditioner.quantity - 1) {
  ...
  addToCart() {
    ...
    this.customerService.getByUserName(localStorage.getItem('token')).subscribe(customer => {
      ...
      this.order.productID = this.airConditioner.id;
      ...
      this.orderService.getByCustomerID(customer.id).subscribe(order => {
        if (order == null) {
          ...
          this.orderService.add(this.order).subscribe(data => {
            this.airConditioner.quantity -= this.quantitySelected;
            swal({
              type: 'success',
              title: 'Đã thêm sản phẩm ' + this.airConditioner.name + ' thành công!'
            });
            this.quantitySelected = 1;
          }, error => {
            swal({
              type: 'error',
              title: 'Thêm sản phẩm ' + this.airConditioner.name + ' thất bại!'
            });
